<?php
function search_courses($college_name)
{
	...
	foreach($array_final_courses_paras as $course_para)
	{
		...
		{
			$file_display_name_temp=explode("/",$file);
			$file_name_with_ext=$file_display_name_temp[1];
			$file_ext_arr=explode(".",$file_name_with_ext);
			$file_display_name=$file_ext_arr[0];
			$file_display_name=strtoupper($file_display_name);
			...
			foreach($arr_course_names as $course_name)
			{
				...
				if($clean_course_para!="" &&$clean_course_name!="" && $percent>=80)
				{
					if(!isset($courses_to_display[$file_display_name]))
						$courses_to_display[$file_display_name]=array();
					array_push($courses_to_display[$file_display_name],$course_name);
	...
	$final_courses_to_display=array();
	foreach($courses_to_display as $dept=>$courses)
	{
		$courses=array_unique($courses);
		$final_courses_to_display[$dept]=$courses;			
		
	}
	return $final_courses_to_display;
	
}//function get_courses($college_name)
